...
def authenticate_user(username, password):
    ...
    if username == 'admin':
        if verify_password(password, users_data.get('admin_password', hash_password(ADMIN_PASSWORD))):
            expires_at = datetime.now(timezone.utc) + timedelta(days=365)  # 1 year for admin
...
def verify_user_token(token):
    ...
    # For admin, just verify the token is valid (no expiration, no auto-logout)
    if username == 'admin':
        return True, payload
    ...
    for user in users_data['users']:
        if user['username'] == username:
            ...
            # Allow expired accounts to continue using the system - no auto-logout
            break

    if not user_found:
        return False, "User not found"

    # Very lenient session management - never auto-remove sessions for any user
    sessions_data = load_sessions()
    session_found = False
