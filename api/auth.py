...
def update_user_stats(username, cookies_checked=0):
    try:
        ...
        for user in users_data['users']:
            if user['username'] == username:
                user['total_checks'] = int(user.get('total_checks', 0)) + 1
                user['total_cookies'] = int(user.get('total_cookies', 0)) + int(cookies_checked)
                save_users(users_data)
                return True
        return False
    except Exception as e:
        print(f"Error updating user stats for {username}: {e}")
        return False
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            if path == '/api/auth/login':
            ...
            elif path == '/api/auth/update_stats':
                ...
                username = payload.get('username')
                if username == 'admin':
                    raise ValueError("Admin cannot update stats this way")

                cookies_checked = int(data.get('cookies_checked', 0))

                if cookies_checked > 0:
                    success = update_user_stats(username, cookies_checked)
                    if success:
                        response = {
                            'success': True,
                    ...
                    response = {
                        'success': False,
                        'message': 'Invalid cookies_checked value'
                    }
    ...
    def do_GET(self):
        try:
            ...
            if path == '/api/auth/users':
                ...
                # Allow users to get their own stats
                token = self.headers.get('Authorization', '').replace('Bearer ', '')
                ...
                username = payload.get('username')
                if username == 'admin':
                    raise ValueError("Use admin endpoint for admin stats")
