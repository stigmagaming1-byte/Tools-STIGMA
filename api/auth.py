...
# Defaults kept for local development but change them before deploy.
# Secrets must come from environment variables in production. Do NOT commit secrets.
# In local development you can set SECRET_KEY in your environment or use a .env file (not committed).
SECRET_KEY = os.environ.get('SECRET_KEY')
ADMIN_PASSWORD = os.environ.get('ADMIN_PASSWORD', 'thnksadmin')

# MongoDB connection
MONGODB_URI = os.environ.get('MONGODB_URI', 'mongodb+srv://stigma:<db_password>@cluster0.co6zdri.mongodb.net/?appName=Cluster0')
...
# Use project-local `data/` by default so it's consistent across platforms.
# For production (e.g. Vercel) don't rely on file storage - use MongoDB.
DATA_DIR = os.environ.get('COOKIE_CHECKER_DATA_DIR', os.path.join(os.getcwd(), 'data'))
...
def verify_user_token(token):
    ...
    # Check if user exists first (don't check active status for verification - only for login)
    users_data = load_users()
    user_found = False
    user_data = None
    for user in users_data['users']:
    ...
    # VERY LENIENT APPROACH: Allow access even if expired or deactivated
    # Only admin can manually deactivate/delete users
    # This ensures user data persists and only admin can remove it

    # Always allow token verification for existing users
    # Update last activity if user data exists
    if user_data:
        sessions_data = load_sessions()
        session_found = False
        current_time = datetime.now(timezone.utc)
        ...
        for session in sessions_data['sessions']:
            if session['token'] == token:
                ...
                if last_activity:
                    ...
                    if (current_time - last_activity_dt).seconds > 300:
                        session['last_activity'] = datetime.now(timezone.utc).isoformat()
                        save_sessions(sessions_data)
                break

        # If no session found, create one (always allow for persistence)
