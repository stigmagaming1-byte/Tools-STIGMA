...
def verify_user_token(token):
    ...
    for user in users_data['users']:
        if user['username'] == username:
            ...
            # NO AUTO DEACTIVATION - Account remains active even if expired for manual admin management
            break

    if not user_found:
        return False, "User not found"

    # Check session validity - very lenient approach for multi-account support
    sessions_data = load_sessions()
    session_found = False
    current_time = datetime.now(timezone.utc)
    ...
    # If no session found, create one (lenient approach for multi-account)
