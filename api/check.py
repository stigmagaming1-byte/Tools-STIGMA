...
# Import dari auth.py untuk update user stats
    ...
    def do_POST(self):
        try:
            ...
            if action == 'start':
                ...
                success, payload = verify_user_token(token)
                if not success:
                    raise ValueError("Invalid token")

                username = payload.get('username')
                if username == 'admin':
                    raise ValueError("Admin cannot check cookies")
                ...
                thread = threading.Thread(target=check_cookies_batch, args=(cookies, username))
                thread.daemon = True
                thread.start()
                ...
                # Update user stats in database for single test
                try:
                    update_user_stats(username, 1, 1)  # 1 check, 1 cookie
                except Exception as e:
                    print(f"Failed to update user stats for {username}: {e}")
...
def check_cookies_batch(cookies, username=None):
    ...
    if checker_state['is_checking']:
        ...
        # Update user stats after batch completion
        if username:
            try:
                valid_count = checker_state['live_data']['valid']
                total_checked = checker_state['live_data']['total_checked']
                update_user_stats(username, valid_count, total_checked)
                print(f"Updated stats for user {username}: {valid_count} valid out of {total_checked}")
            except Exception as e:
