    ...
    try {
        ...
        if (data.success) {
            userData = {
                ...userData,
                ...data.user,
                days_left: data.user.days_left || 0,
                login_count: data.user.login_count || 1,
                total_checks: data.user.total_checks || 0,
                total_cookies: data.user.total_cookies || 0
            };

            // Update localStorage as backup
            localStorage.setItem('user_total_checks', userData.total_checks);
            localStorage.setItem('user_total_cookies', userData.total_cookies);
        } else {
            console.error('Failed to load user stats:', data.message);
            // Fallback to localStorage if API fails
            userData = {
                ...userData,
                days_left: userData.days_left || 0,
