...
async function verifyToken() {
    ...
    try {
        ...
        if (data.success) {
            ...
            // DON'T clear localStorage - allow persistent access
            // User data should persist and only be removed by admin
            console.warn('Token verification failed, but keeping localStorage for persistence');
            // Try to restore user data from localStorage if available
            currentUser = localStorage.getItem('cookieCheckerUser');
            userToken = token;
            return true; // Allow access anyway for data persistence
        }
    } catch (error) {
        console.error('Token verification error:', error);
        // DON'T clear localStorage on error - maintain persistence
        console.warn('Token verification error, but keeping localStorage for persistence');
        // Try to restore user data from localStorage if available
        currentUser = localStorage.getItem('cookieCheckerUser');
        userToken = token;
        return true; // Allow access anyway for data persistence
...
function addResultToTable(result) {
    ...
    const avatarUrl = result.user_id ? `https://www.roblox.com/headshot-thumbnail/image?userId=${result.user_id}&width=150&height=150&format=png` : '';

    row.innerHTML = `
        <td>${result.cookie_id + 1}</td>
        <td class="avatar-cell">${avatarUrl ? `<img src="${avatarUrl}" alt="Avatar" class="user-avatar" onerror="this.src='data:image/svg+xml;base64,'; this.onerror=null;">` : '<div class="avatar-placeholder"><i class="fas fa-user"></i></div>'}</td>
        <td>${statusBadge}</td>
...
function updateLogsList(logs) {
    ...
    logs.slice(0, 20).forEach(log => {
        ...
        // Construct avatar URL for valid accounts
        const avatarUrl = (log.status === 'valid' && log.user_id) ?
            `https://www.roblox.com/headshot-thumbnail/image?userId=${log.user_id}&width=50&height=50&format=png` : '';

        logItem.innerHTML = `
            <div class="log-header">
                ${avatarUrl ? `<img src="${avatarUrl}" alt="Avatar" class="log-avatar" onerror="this.style.display='none';">` : '<div class="log-avatar-placeholder"><i class="fas fa-user"></i></div>'}
                <div class="log-info">
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (isLoggedIn) {
        ...
        // Force navigation for login links
        const loginLinks = document.querySelectorAll('.nav-login, .nav-login-mobile, .btn-ad');
        console.log('Found login links:', loginLinks.length);
        loginLinks.forEach((link, index) => {
            console.log(`Attaching listener to link ${index}:`, link);
            link.addEventListener('click', function(e) {
                console.log('Login link clicked, preventing default and redirecting to login.html');
                e.preventDefault();
                e.stopPropagation();
