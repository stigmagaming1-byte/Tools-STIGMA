...
// Auto-save and auto-lock variables
...
let savedCookies = [];
let savedResults = [];
let savedStats = { valid: 0, invalid: 0, robux: 0, premium: 0 };
...
cookiesInput.addEventListener('input', handleUserActivity);
    ...
    try {
        ...
        if (data.success) {
            ...
            if (data.data.role === 'admin') {
                ...
                return false; // Don't proceed with user dashboard
            }
...
function resetAutoLockTimer() {
    ...
    autoLockTimeout = setTimeout(() => {
        if (!isChecking) { // Don't auto-lock while checking
            lockDashboard();
        }
...
function lockDashboard() {
    ...
    showToast('Dashboard terkunci karena tidak aktif', 'warning');
}
...
function unlockDashboard() {
    ...
    showToast('Dashboard terbuka kembali', 'success');
}
    ...
    if (!overlay) {
        ...
        overlay.innerHTML = `
            <div class="lock-content">
                ...
                <h3>Dashboard Terkunci</h3>
                <p>Klik di mana saja untuk membuka kembali</p>
            </div>
        `;
        overlay.style.cssText = `
            ...
            cursor: pointer;
        `;
        overlay.addEventListener('click', unlockDashboard);
...
function saveCurrentState() {
    // Save cookies
    savedCookies = parseCookies(cookiesInput.value);

    // Save results
    savedResults = Array.from(resultsBody.querySelectorAll('tr')).map(row => {
        const cells = row.querySelectorAll('td');
        return {
            id: cells[0].textContent,
            status: cells[1].textContent,
            username: cells[2].textContent,
            displayName: cells[3].textContent,
            robux: cells[4].textContent,
            premium: cells[5].innerHTML.includes('fa-crown'),
            friends: cells[6].textContent,
    ...
    // Save stats
    savedStats = {
        valid: parseInt(validCount.textContent) || 0,
        invalid: parseInt(invalidCount.textContent) || 0,
        robux: parseInt(totalRobux.textContent.replace(/,/g, '')) || 0,
        premium: parseInt(premiumCount.textContent) || 0
    };

    // Save to localStorage
    localStorage.setItem('autoSave_cookies', JSON.stringify(savedCookies));
    localStorage.setItem('autoSave_results', JSON.stringify(savedResults));
    localStorage.setItem('autoSave_stats', JSON.stringify(savedStats));
    localStorage.setItem('autoSave_timestamp', Date.now());
}

function restoreSavedState() {
    try {
        const savedCookiesData = localStorage.getItem('autoSave_cookies');
        const savedResultsData = localStorage.getItem('autoSave_results');
        const savedStatsData = localStorage.getItem('autoSave_stats');

        if (savedCookiesData) {
            const cookies = JSON.parse(savedCookiesData);
            cookiesInput.value = cookies.join('\n');
            updateCookieCount();
        }

        if (savedResultsData) {
            const results = JSON.parse(savedResultsData);
            results.forEach(result => {
                addResultToTable(result);
            });
            noResults.style.display = results.length > 0 ? 'none' : 'flex';
        }

        if (savedStatsData) {
            const stats = JSON.parse(savedStatsData);
            validCount.textContent = stats.valid;
            invalidCount.textContent = stats.invalid;
            totalRobux.textContent = stats.robux.toLocaleString();
            premiumCount.textContent = stats.premium;
        }

        showToast('Data tersimpan telah dipulihkan', 'info');
    } catch (error) {
