...
const dataLockStatus = document.getElementById('dataLockStatus');
...
refreshLogs.addEventListener('click', loadGlobalLogs);
...
restoreDataBtn.addEventListener('click', restoreFromBackup);
...
async function loadGlobalLogs() {
    try {
        const response = await fetch(`${API_BASE_URL}/api/global/logs`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${adminToken}`,
                'Accept': 'application/json'
        ...
        console.error('Load global logs error:', error);
        showAdminToast('Error memuat global logs', 'error');
    }
}

async function loadGlobalResults() {
    try {
        const response = await fetch(`${API_BASE_URL}/api/global/results`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${adminToken}`,
                'Accept': 'application/json'
        ...
        if (data.success) {
            renderResultsLogs(data.results);
            updateLogsStats(data.stats);
        }
    } catch (error) {
        console.error('Load global results error:', error);
        showAdminToast('Error memuat global results', 'error');
    }
}
...
function renderActivityLogs(logs) {
    ...
    logs.forEach(log => {
        ...
        activityLogsList.appendChild(logItem);
    });
}

function renderResultsLogs(results) {
    resultsLogsList.innerHTML = '';

    if (!results || results.length === 0) {
        resultsLogsList.innerHTML = `
            <div class="log-item empty">
                <i class="fas fa-inbox"></i>
                <p>No check results found</p>
            </div>
        `;
        return;
    }
    ...
    results.forEach(result => {
        ...
        const statusIcon = status === 'valid' ? 'fas fa-check-circle' : 'fas fa-times-circle';
        ...
        logItem.innerHTML = `
            ...
            <div class="log-content">
                <div class="log-header">
                    <span class="log-username">${result.username}</span>
                    <span class="log-status ${statusClass}">${status.toUpperCase()}</span>
                    <span class="log-time">${timestamp}</span>
                </div>
                ${result.result.robux !== undefined ? `<div class="log-robux">Robux: ${result.result.robux}</div>` : ''}
                ${result.result.details ? `<div class="log-details">${result.result.details}</div>` : ''}
            </div>
        `;

        resultsLogsList.appendChild(logItem);
    });
}

function updateLogsStats(stats) {
    if (stats) {
        totalLogs.textContent = stats.total_logs || 0;
        validResults.textContent = stats.total_valid_cookies || 0;
        totalRobux.textContent = stats.total_robux_found || 0;
        activeUsersLog.textContent = stats.active_users || 0;
    }
}
...
function filterLogs() {
    ...
    const logItems = document.querySelectorAll('.logs-list .log-item:not(.empty):not(.loading)');

    logItems.forEach(item => {
        const username = item.querySelector('.log-username')?.textContent.toLowerCase() || '';
        const action = item.querySelector('.log-action')?.textContent.toLowerCase() || '';
        const status = item.querySelector('.log-status')?.textContent.toLowerCase() || '';

        const matchesFilter = !filterValue || action.includes(filterValue.toLowerCase()) || status.includes(filterValue.toLowerCase());
        const matchesSearch = !searchValue || username.includes(searchValue) || action.includes(searchValue);
...
function switchLogsTab(tabId) {
    logsTabButtons.forEach(button => {
        button.classList.remove('active');
        if (button.getAttribute('data-log-tab') === tabId) {
            button.classList.add('active');
        }
    });

    const tabContents = document.querySelectorAll('.logs-tab-content');
    tabContents.forEach(content => {
        content.classList.remove('active');
        if (content.id === `${tabId}-logs-tab`) {
            content.classList.add('active');
        }
    });
...
function getActionIcon(action) {
    switch(action.toLowerCase()) {
        ...
        case 'check': return 'fas fa-cookie-bite';
        case 'create': return 'fas fa-user-plus';
        case 'extend': return 'fas fa-calendar-plus';
...
async function saveAllData() {
    if (!confirm('Are you sure you want to save all current data? This will persist all logs and results.')) {
        return;
    }

    try {
        showAdminToast('Saving data...', 'info');

        const response = await fetch(`${API_BASE_URL}/api/admin/save_data`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${adminToken}`,
                'Content-Type': 'application/json'
        ...
        if (data.success) {
            showAdminToast('Data saved successfully!', 'success');
            updateLastSavedTime();
            updateDataLockStatus('Locked');
        } else {
            showAdminToast(data.message || 'Failed to save data', 'error');
        }
    } catch (error) {
        console.error('Save data error:', error);
        showAdminToast('Error saving data: ' + error.message, 'error');
    }
}

async function createBackup() {
    try {
        showAdminToast('Creating backup...', 'info');
        ...
        if (data.success) {
            ...
            showAdminToast('Backup created successfully!', 'success');
        } else {
            showAdminToast(data.message || 'Failed to create backup', 'error');
        }
    } catch (error) {
        console.error('Create backup error:', error);
        showAdminToast('Error creating backup: ' + error.message, 'error');
    }
}
...
async function restoreFromBackup() {
    ...
    input.onchange = async (e) => {
        ...
        try {
            showAdminToast('Restoring from backup...', 'info');
            ...
            if (data.success) {
                ...
                showAdminToast(data.message || 'Failed to restore data', 'error');
            }
        } catch (error) {
            console.error('Restore backup error:', error);
            showAdminToast('Error restoring backup: ' + error.message, 'error');
        }
    };
...
function updateLastSavedTime() {
    const now = new Date();
    lastSavedTime.textContent = now.toLocaleString('id-ID');
}

function updateDataLockStatus(status) {
    dataLockStatus.textContent = status;
}
