...
def api_check():
    ...
    try:
        if action == 'start':
            # Queue the job instead of starting a background thread (serverless-friendly)
            ...
            try:
                if hasattr(auth_mod, 'db') and auth_mod.db:
                    auth_mod.db.jobs.replace_one({'job_id': job_id}, job, upsert=True)
                else:
                    jobs_file = os.path.join(auth_mod.DATA_DIR, 'jobs.json')
                    jobs = []
                    if os.path.exists(jobs_file):
                        try:
                            with open(jobs_file, 'r') as jf:
                                jobs = json.load(jf)
                        except:
                            jobs = []
                    jobs.append(job)
                    with open(jobs_file, 'w') as jf:
                        json.dump(jobs, jf, indent=2)
            except Exception as e:
                print('Job queue save error:', e)

            return jsonify({'success': True, 'message': f'Job queued with id {job_id}', 'job_id': job_id, 'total': len(cookies)})
    ...
    @app.route('/api/check/process_job', methods=['POST'])
    def api_process_job():
        """Process one queued job synchronously. If `job_id` is provided in JSON body, process that job,
        otherwise pick the oldest pending job. This allows serverless functions (Vercel) to run jobs
        ...
        try:
            if hasattr(auth_mod, 'db') and auth_mod.db:
                if job_id:
                    job = auth_mod.db.jobs.find_one({'job_id': job_id})
                else:
                    job = auth_mod.db.jobs.find_one({'status': 'pending'})
            else:
                jobs_file = os.path.join(auth_mod.DATA_DIR, 'jobs.json')
                jobs = []
                if os.path.exists(jobs_file):
            ...
            return jsonify({'success': False, 'message': f'Error loading job: {e}'}), 500

        if not job:
            return jsonify({'success': False, 'message': 'No pending job found'}), 404
        ...
        try:
            if hasattr(auth_mod, 'db') and auth_mod.db:
                auth_mod.db.jobs.replace_one({'job_id': job['job_id']}, job, upsert=True)
            else:
                # update file
                jobs_file = os.path.join(auth_mod.DATA_DIR, 'jobs.json')
                jobs = []
            ...
            # Prepare checker state and run synchronously
            ...
            check_mod.checker_state['live_data'] = {
                ...
                'progress': 0,
                'current': 1,
                'total': len(job.get('cookies', [])),
                'start_time': time.time()
            }

            # Run the batch checker synchronously (may take time — ensure Vercel timeout allows)
            check_mod.check_cookies_batch(job.get('cookies', []))

            # Collect results
            results = check_mod.checker_state.get('results', [])
            job['results'] = results
        ...
        return jsonify({'success': True, 'job_id': job.get('job_id'), 'status': job.get('status'), 'total_results': len(job.get('results', []))})
